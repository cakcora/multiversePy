...
def process_data(prepped_data, name, config, feature_name):
    ...
    result.to_csv(f'{config["out_csv_dir"]}results_fake{name}.csv', mode='a', header=False, index=False)
...
def main():
    ...
    for i in range(len(configs_preprocessing)):
        ...
        col = np.random.randint(0, 100, size=(32561, 1))
        # raw['rand'] = np.random.randint(0, 100, size=(32561, 1)))
        raw.insert(loc=len(raw.columns) - 1, column='rand', value=col)  # Insert column
