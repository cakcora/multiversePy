...
def run_dataset(config, feature_name):
    ...
    result_process_data = process_data(data, name, config, feature_name)
...
def main():
    ...
    for i in range(len(configs_preprocessing)):
        ...
        result_run_dataset_vf = run_dataset(config, "Vanilla")
        print("Running for {} is completed \n".format("Vanilla"))
        num_features = len(raw.columns)
        feature_importance(result_run_dataset_vf[0], result_run_dataset_vf[1], num_features, raw)
        ...
        # num_features = len(raw.columns)
        # feature_importance(result_run_dataset_vf[0], result_run_dataset_vf[1], num_features)
